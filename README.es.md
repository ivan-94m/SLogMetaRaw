...
1. Descarga `SLogMetaRaw-2.0.0.dmg` desde **Releases** y ábrelo.
...
| Control | Qué hace |
|---|---|
| **Versión** (arriba) | `v2.0.0`. Una vez al día pregunta a GitHub por la última release. Si la hay muestra **🟢 v2.0.0 → 2.x.y** y el clic abre la descarga del DMG en el navegador. Nunca instala nada por sí mismo |
...
- Los **nodos** preguntan a GitHub por la última release de este proyecto como mucho una vez al día, en segundo plano.
  La petición lleva solo la versión del programa (`User-Agent: SLogMetaRaw/2.0.0`). Para desactivarlo crea el archivo
