...
1. Download `SLogMetaRaw-2.0.1.dmg` from **Releases** and open it.
...
|---|---|
| **Version** (top) | `v2.0.1`. Once a day it asks GitHub for the latest release. If there is one it reads **🟢 v2.0.1 → 2.x.y**, and a click opens the DMG download in the browser. It never installs anything itself |
...
- The **nodes** ask GitHub for this project's latest release at most once a day, in the background. The request
  carries only the program version (`User-Agent: SLogMetaRaw/2.0.1`). To turn it off, create the empty file
