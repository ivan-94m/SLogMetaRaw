...
1. Baixe `SLogMetaRaw-2.1.1.dmg` em **Releases** e abra-o.
...
| Controle | O que faz |
|---|---|
| **Versão** (no topo) | `v2.1.1`. Uma vez por dia pergunta ao GitHub pela última release. Se houver, mostra **🟢 v2.1.1 → 2.x.y** e o clique abre o download do DMG no navegador. Nunca instala nada sozinho |
...
- Os **nós** perguntam ao GitHub pela última release deste projeto no máximo uma vez por dia, em segundo plano. A
  requisição leva só a versão do programa (`User-Agent: SLogMetaRaw/2.1.1`). Para desativar, crie o arquivo vazio
